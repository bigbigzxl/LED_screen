...
void Display::fadeInString(unsigned char* s, uint8_t delta)
{
    if (last_fade_delta == 0)
    {
        m_canvasBuffer = m_canvasBuffer_ping;
        for (uint16_t x = 0; x < m_width; x++)
        {
    ...
    last_fade_delta += delta;
    if (last_fade_delta < 255)
    {
        memcpy(m_canvasBuffer_ping, m_canvasBuffer_pong, m_width*m_height*sizeof(CRGB));
        fadeToBlackBy(m_canvasBuffer_pong, m_width*m_height, 255 - last_fade_delta);
    }
    else
    {
        last_fade_delta = 0;
